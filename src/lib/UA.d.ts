...
export class UA extends EventEmitter {
  ...
  // options call active
  speaker(voice: boolean): RTCSession;

  transfer(target: string): RTCSession;

  sendDTMF(tone: string): RTCSession;

  mute(audio: boolean, video: boolean): RTCSession;

  unmute(audio: boolean, video: boolean): RTCSession;
