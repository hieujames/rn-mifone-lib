...
module.exports = class RTCSession extends EventEmitter {
  ...
  connect(target, options = {}, initCallback) {
    ...
    console.log('======>   ' + JSON.stringify(eventHandlers));
    ...
    Promise.resolve()
      .then(() => {
        ...
        // Audio and/or video requested, prompt getUserMedia.
          ...
          return mediaDevices.getUserMedia(mediaConstraints).catch(error => {
            if (this._status === C.STATUS_TERMINATED) {
  ...
  terminate(options = {}) {
    ...
    console.log('[CHECK] RTC_terminate =>  ' + JSON.stringify(this._status));
  ...
  mute(options = {audio: true, video: false}) {
    ...
    console.log('[CHECK] OPTIONS =>  ' + JSON.stringify(this._connection));
  ...
  refer(target, options) {
    logger.debug('refer()');
    console.log('[CHECK] refer=>  ' + target);
    const originalTarget = target;
  ...
  sendRequest(method, options) {
    ...
    console.log('[CHECK] Send Request (method, options)');
  ...
  _createRTCConnection(pcConfig, rtcConstraints) {
    ...
    console.log('[RTCConnection] ' + JSON.stringify(this._connection));
  ...
  _createDialog(message, type, early) {
    ...
    console.log(`[CHECK]  DIALOG:  ${message}`);
    // Early Dialog.
    if (early) {
      if (early_dialog) {
        return true;
  ...
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream) {
    ...
    console.log(
      '[CHECK] mediaConstraints (1):  ' + JSON.stringify(mediaConstraints),
    );
    ...
    Promise.resolve()
      ...
      .then(() => {
        ...
        // Request for user media access.
          ...
          this._localMediaStreamLocallyGenerated = true;
          console.log('_sendInitialRequest 1 ');
          //console.log('_sendInitialRequest get mediaContrains:  ' + JSON.stringify(mediaConstraints));
          return mediaDevices.getUserMedia(mediaConstraints).catch(error => {
            if (this._status === C.STATUS_TERMINATED) {
              throw new Error('terminated');
            }
            console.log('_sendInitialRequest getUserMedia 0 ');
            this._failed(
              'local',
              null,
              JsSIP_C.causes.USER_DENIED_MEDIA_ACCESS,
      ...
      .then(desc => {
        console.log('_sendInitialRequest 3 ');
      ...
      .catch(error => {
        ...
        console.log('_sendInitialRequest ERROR ' + error);
        logger.warn(error);
      });
  }
  ...
  _receiveInviteResponse(response) {
    ...
    console.log('[CHECK]  Received invite response');
    ...
    console.log('[CHECK]  STATUS:  ' + this._status);
    ...
    switch (true) {
      ...
      case /^2[0-9]{2}$/.test(response.status_code): {
        ...
        logger.debug('emit "sdp"');
        console.log(`[CHECK] debug emit sdp:  ${JSON.stringify(e)}`);
        this.emit('sdp', e);
  ...
  _toggleMuteAudio(mute) {
    console.log('[CHECK] _toggleMuteAudio:  ' + mute);
    // if (!this._connection) {
    //   return;
    // }

    const senders = this._connection.getTransceivers().filter(sender => {
      return sender.track && sender.track.kind === 'audio';
    });

    console.log('[CHECK] _toggleMuteAudio:  ' + JSON.stringify(senders));
    ...
    //   const audioTracks = this._localMediaStream.getAudioTracks();
  ...
  _newRTCSession(originator, request) {
    ...
    console.log('[_newRTCSession]');
    console.log('[_newRTCSession] FUNC ' + request);
