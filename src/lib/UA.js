...
module.exports = class UA extends EventEmitter {
  ...
  mute(audio, video) {
    ...
    const session = new RTCSession(this);
    session.mute({audio: audio, video: video});
  ...
  unmute(audio, video) {
    ...
    const session = new RTCSession(this);
    session.unmute({audio: audio, video: video});
  ...
  hangup(options) {
    console.log('hangup call' + JSON.stringify(options));
  ...
  _loadConfig(configuration) {
    ...
    // Contact URI.
      ...
      this._configuration.via_host = configuration.domain;
      this._configuration.contact_uri = new URI(
        'sip',
        this._configuration.display_name,
    ...
    console.log(`CONTACT HEADER:  ${this.configuration.contact_uri}`);
